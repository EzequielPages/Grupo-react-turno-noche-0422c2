  ...
  return (
    ...
    <div className="border border-gray-600 shadow-sm bg-white flex flex-col justify-start card-size  md:bg-white-500 " >
      <div className="containerAnimated">
        <img className="overflow-hidden size-image imageAnimated" src={image} alt={image}/> 
          <div class="overlayAnimated">
              <div class="textAnimated italic text-sm"><FontAwesomeIcon icon={faLocationDot}/> {textDeploy}
              </div>
          </div>
      </div>
        <div>
        ...
        <span className="m-1 text-gray-400">{duration}</span><span className="text-green-600">USD${price}</span>
        </div>

        </div>  
        <button className="hover:btn-primary transition-all duration-1000 border btn-style h-20 italic" >MÁS INFORMACIÓN
        </button>
